...
<head>
    ...
    <title>S&#7917;a v&#7883; tr&#237; &#273;&#7895; - Admin</title>
...
<body class="pm-dashboard-layout">
    ...
    <div class="pm-dash-body">
        ...
        <main class="pm-dash-main">
            <h2 class="pm-dash-content-title">S&#7917;a v&#7883; tr&#237; &#273;&#7895; xe</h2>
            <% if (error != null) { %><p class="pm-error" style="max-width:400px;"><%= error %></p><% } %>
            <div class="pm-dash-table-wrap" style="max-width:480px;">
                <form action="<%= ctx %>/admin/slot?action=edit" method="post" class="pm-form" style="padding:24px;">
                    <input type="hidden" name="action" value="edit">
                    <input type="hidden" name="slotId" value="<%= slot.getSlotId() %>">
                    <div class="pm-form-group">
                        <label>M&#227; slot</label>
                        <input type="text" value="<%= slot.getSlotId() %>" readonly style="background:#f1f5f9;">
                    </div>
                    <div class="pm-form-group">
                        <label>B&#227;i xe</label>
                        <select name="lotId" required>
                            <% for (ParkingLot lot : lotList) { %>
                            <option value="<%= lot.getLotId() %>" <%= lot.getLotId().equals(slot.getLotId()) ? "selected" : "" %>><%= lot.getLotName() %> (<%= lot.getLotId() %>)</option>
                            <% } %>
                        </select>
                    </div>
                    <div class="pm-form-group">
                        <label>M&#227; &#244; (slotCode)</label>
                        <input type="text" name="slotCode" value="<%= slot.getSlotCode() != null ? slot.getSlotCode() : "" %>" required>
                    </div>
                    <div class="pm-form-group">
                        <label>Tr&#7841;ng th&#225;i</label>
                        <select name="status">
                            <option value="EMPTY" <%= "EMPTY".equals(slot.getStatus()) ? "selected" : "" %>>Tr&#7889;ng</option>
                            <option value="OCCUPIED" <%= "OCCUPIED".equals(slot.getStatus()) ? "selected" : "" %>>&#272;&#227; c&#243; xe</option>
                            <option value="DISABLED" <%= "DISABLED".equals(slot.getStatus()) ? "selected" : "" %>>B&#7843;o tr&#236;</option>
                        </select>
                    </div>
                    <div class="pm-form-group">
                        <label>Ghi ch&#250;</label>
                        <input type="text" name="note" value="<%= slot.getNote() != null ? slot.getNote() : "" %>">
                    </div>
